...
JSFX.Layer = function (newLayer) {
  ...
  if (typeof newLayer == 'string') this.elem = JSFX.createElem(newLayer)
...
/**********************************************************************************/
var ns4 =
  navigator.appName.indexOf('Netscape') != -1 && !document.getElementById
...
if (document.all) {
  JSFX.Layer.prototype.setOpacity = function (pc) {
    if (this.style.filter == '') this.style.filter = 'alpha(opacity=100);'
...
JSFX.AddGhost = function (img) {
  var myGhost = null
  var htmlStr = "<IMG SRC='" + img + "' BORDER='0' WIDTH='25' HEIGHT='25'>"
...
JSFX.animateGhosts = function () {
  if (this.state == 'off') {
    ...
    if (Math.random() > 0.98) this.state = 'down'
  } else if (this.state == 'up') {
    this.op += 2
    this.setOpacity(this.op)
    if (this.op == 100) this.state = 'on'
  } else if (this.state == 'down') {
    this.op -= 2
    if (this.op == 0) {
      this.hide()
      this.state = 'off'
    } else this.setOpacity(this.op)
  }
  ...
  if (Math.random() > 0.95) {
    ...
    this.targetY =
      Math.random() *
      (JSFX.Browser.getCanvasHeight() + JSFX.Browser.getMinY() - 150)
  }
}
...
for (var i = 0; i < 10; i++) {
  setTimeout(function () {
    JSFX.AddGhost('/assets/img/bat.gif')
  }, delay)
  delay += 500
}
