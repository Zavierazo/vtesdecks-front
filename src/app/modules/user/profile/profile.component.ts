...
export class ProfileComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.form = new FormGroup({
      displayName: new FormControl(this.authQuery.getDisplayName(), Validators.required),
      password: new FormControl(null),
      newPassword: new FormControl(null, [Validators.minLength(8), this.patternValidator(/\d/, { hasNumber: true })]),
      confirmNewPassword: new FormControl(null, [Validators.minLength(8), this.patternValidator(/\d/, { hasNumber: true })]),
    },
      {
    ...
    this.authService.updateSettings(settings)
      .pipe(
        ...
        error: () => {
          ...
          this.message = 'An error occurred while updating your settings.';
          this.changeDetectorRef.detectChanges();
        }
      });
