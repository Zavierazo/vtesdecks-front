...
import { ApiCrypt, ApiI18n, ApiLibrary, ApiSet, ApiWishlistCard } from '@models'
import {
  NgbActiveModal,
  NgbHighlight,
  NgbModal,
...
@UntilDestroy()
@Component({
  selector: 'app-wishlist-card-modal',
  templateUrl: './wishlist-card-modal.component.html',
...
export class WishlistCardModalComponent implements OnInit {
  ...
  loading$ = this.wishlistQuery.selectLoadingBackground()
  ...
  get cardWishlistId(): number | null | undefined {
  ...
  onSubmit() {
    // Enter key submit: update when editing, add otherwise
    if (this.cardWishlistId) {
      this.onEdit()
    } else {
      this.onSave(true)
