...
describe('Deck builder', () => {
  ...
  it('exposes the import options dropdown', () => {
    ...
    cy.get(SEL.ngbDropdownMenu).contains(/amaranth/i).should('exist')
  ...
  it('adding a crypt card increases the crypt counter (relative change)', () => {
    ...
    cy.get('span.circle_number').first().invoke('text').then((before) => {
      ...
      cy.get('span.circle_number')
        ...
        .then((after) => {
          ...
          expect(afterN, 'crypt count increased after adding a card').to.be.greaterThan(beforeN)
        })
    })
  })
